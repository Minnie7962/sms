...
class StudentSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        try {
            ...
            $classes = \App\Models\MyClass::whereNotIn('name', ['Office1', 'Office2', 'Library'])->get();
            ...
            foreach ($classDistribution as $classGroupName => $count) {
                $classGroup = \App\Models\ClassGroup::where('name', $classGroupName)->first();
                if (!$classGroup) {
                    $this->command->error("Class group '$classGroupName' not found.");
                    continue;
                }
                
                $classesInGroup = \App\Models\MyClass::where('class_group_id', $classGroup->id)
    ...
    private function createStudent($school, $classGroup, $class, $section, $gender, $firstName, $lastName, $phonePrefixes, $index, $locations)
    {
        $fullName = "$firstName $lastName";
        $email    = strtolower($firstName . '.' . $lastName . Str::random(3) . '@student.tamat.edu.kh');
        
        $ageOffset = strpos($classGroup->name, 'Kindergarten') !== false 
            ? 6 
            : (6 + (int)filter_var($classGroup->name, FILTER_SANITIZE_NUMBER_INT));
        ...
        // Create parent and emergency contact data
        ...
        // Attach the current academic year record via pivot
        if ($school->academic_year_id) {
            $studentRecord->academicYears()->sync([
